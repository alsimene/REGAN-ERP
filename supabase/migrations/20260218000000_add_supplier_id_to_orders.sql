-- Add supplier_id column to orders (FK to clients table)
ALTER TABLE orders ADD COLUMN supplier_id bigint REFERENCES clients(id);

-- Default all existing orders to Regan Industrial (id=16)
UPDATE orders SET supplier_id = 16;

-- Drop old overloaded versions of create_order_with_items
DROP FUNCTION IF EXISTS public.create_order_with_items(text, bigint, text, text, jsonb);
DROP FUNCTION IF EXISTS public.create_order_with_items(text, bigint, text, text, jsonb, text);
DROP FUNCTION IF EXISTS public.create_order_with_items(text, bigint, text, text, jsonb, text, bigint);

-- Recreate with supplier_id support
